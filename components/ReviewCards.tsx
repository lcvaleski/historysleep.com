...
function StarIcon() {
  return (
    <svg className="w-3 h-3 fill-ms-orchid" viewBox="0 0 20 20">
  ...
  return (
    <div className="flex gap-4 justify-center flex-wrap px-6 sm:px-0">
      {reviews.map((review) => (
        <div
          key={review.author}
          className={`bg-ms-nocturne/95 rounded-[18px] p-8 pb-8 shadow-lg border border-ms-lavendar/10 ${cardClassName}`}
        >
          ...
          <p className="text-sm text-ms-lavendar/60 font-medium">{review.author}</p>
        </div>
      ))}
    </div>
  )
